...
function programaReactivacion(nombre) {
  ...
  if (cliente.historialLimpio && cliente.ingresosEstables) {
    console.log(
      ` ${nombre} califica para el programa de reactivación financiera.`,
    );
  } else {
    console.log(
      ` ${nombre} no califica para el programa de reactivación financiera.`,
    );
  }
}
...
function riesgoCritico() {
  const clienteRiesgoCritico = clientes.some(
    (cal) => cal.historialLimpio && cal.ingresosEstables,
  );
  if (clienteRiesgoCritico) {
    console.log("Alerta: Existe al menos un cliente en riesgo crítico.");
  } else {
    console.log("No hay clientes en riesgo crítico.");
  }
}
riesgoCritico();
...
function certificacionNacional() {
  const perfilCliente = clientes.filter(
    (cal) => cal.historialLimpio && cal.ingresosEstables,
  );
  if (perfilCliente) {
    console.log(
      " Los clientes no son seguros y el banco no es apto para recibir la certificacion",
    );
  } else {
    console.log(
      "los clientes son seguros y El banco nes apto para la certificacion",
    );
  }
}
