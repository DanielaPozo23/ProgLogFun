...
function esZonaUs() {
    const usuariosUs = evalCont.filter(usuario => usuario.zona === 'us-east' || usuario.zona === 'us-west');
...
function esAltaCarga() {
    const usAltacarga = evalCont.filter(usuario => usuario.consultasPorMinuto >= 10000);
    console.log(usAltacarga);
}
esAltaCarga();
//usaNode(x): devuelve si el servicio inclute 'Node' dentro de su arreglo de tecnologías.
function usaNode() {
    const Node = evalCont.filter(usuario => usuario.tecnologias.includes('Node'));
    console.log(Node);
} usaNode();
...
function requiMantenimientoUrge() {
    const mantenimientoUrg = evalCont.filter(usuario => !usuario.activo && usuario.consultasPorMinuto >= 10000);
...
function esServicioCriticoUs() {
    const servicioCriticoUs = evalCont.filter(usuario => usuario.activo && (usuario.zona === 'us-east' || usuario.zona === 'us-west' || usuario.consultasPorMinuto >= 10000));
...
function migrarACloudFlare() {
    const migrarCloudFlare = evalCont.filter(usuario => (usuario.zona === 'us-east' || usuario.zona === 'us-west') && usuario.tecnologias.includes('Node') && !(usuario.consultasPorMinuto >= 10000));
...
function nomServisCriticoUs() {
    const nombresServicioCriticoUs = evalCont.filter(usuario => usuario.activo && (usuario.zona === 'us-east' || usuario.zona === 'us-west' || usuario.consultasPorMinuto >= 10000)).map(usuario => usuario.nombre);
...
function nomServisMantenimientoUrgente() {
    const nombresServicioMantUrgente = evalCont.filter(usuario => !usuario.activo && usuario.consultasPorMinuto >= 10000).map(usuario => usuario.nombre);
...
function totalConsultasActivos() {
    const totalConsultas = evalCont.filter(usuario => usuario.activo).reduce((total, usuario) => total + usuario.consultasPorMinuto, 0);
