...
function usuarioEspecial() {
  const usuariosEspeciales = hechos.filter(
    (usuario) => usuario.rol === "admin" && usuario.edad < 18,
  );
...
function usuariosConPerEd() {
  const usuariosConPermiso = hechos.filter(
    (usuario) =>
      usuario.activo || usuario.rol === "admin" || usuario.edad >= 18,
  );
